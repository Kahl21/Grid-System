...
public static class FightHandler
{
    ...
    static public void AttackEnemy(Character attacker, Character target)
    {
        ...
        if(attacker.Offense.Accuracy > randNum || randNum == 0)
        {
            ...
            {
                HistoryHandler.AddToCurrentAction("and CRITS for " + target.Defense.CalculateDamage(damage, attacker.HeldWeapon.WeaponElement).ToString() + "\n");
                target.TakeDamage(damage, attacker.HeldWeapon.WeaponElement);
                target.LastAttacker = attacker;
            }
            else
            {
                HistoryHandler.AddToCurrentAction("and HITS for " + target.Defense.CalculateDamage(damage*2, attacker.HeldWeapon.WeaponElement).ToString() + "\n");
                target.TakeDamage(damage * 2, attacker.HeldWeapon.WeaponElement);
                target.LastAttacker = attacker;
            }
        }
        else
